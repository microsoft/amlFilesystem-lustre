...
AS_IF([test $ENABLEO2IB != "no"], [
	...
	#
	# LN_O2IB_IB_DMA_PCI_P2P_DMA_SUPPORTED
	#
	AC_DEFUN([LN_SRC_O2IB_IB_DMA_PCI_P2P_DMA_SUPPORTED], [
		LB2_OFED_TEST_SRC([ib_dma_pci_p2p_dma_supported], [
			#include <rdma/ib_verbs.h>
		],[
			ib_dma_pci_p2p_dma_supported(NULL);
		],[],[$EXTRA_OFED_CONFIG $EXTRA_OFED_INCLUDE])
	])
	AC_DEFUN([LN_O2IB_IB_DMA_PCI_P2P_DMA_SUPPORTED], [
		LB2_OFED_TEST_RESULTS(
			[ib_dma_pci_p2p_dma_supported exists],
			[ib_dma_pci_p2p_dma_supported],
			[HAVE_OFED_IB_DMA_PCI_P2P_DMA_SUPPORTED])
	]) # LN_O2IB_IB_DMA_PCI_P2P_DMA_SUPPORTED
	...
	AC_DEFUN([LN_CONFIG_O2IB_SRC], [
		...
		LN_SRC_O2IB_IB_DMA_PCI_P2P_DMA_SUPPORTED
	])
	AC_DEFUN([LN_CONFIG_O2IB_RESULTS], [
...
#
AC_DEFUN([LN_SRC_IS_PCI_P2PDMA_PAGE], [
	LB2_LINUX_TEST_SRC([is_pci_p2pdma_page], [
		#include <linux/pci-p2pdma.h>
	],[
		is_pci_p2pdma_page(NULL);
	],[-Werror])
])
AC_DEFUN([LN_IS_PCI_P2PDMA_PAGE], [
	LB2_MSG_LINUX_TEST_RESULT([if is_pci_p2pdma_page exists],
	[is_pci_p2pdma_page], [
		AC_DEFINE(HAVE_IS_PCI_P2PDMA_PAGE, 1,
			  [is_pci_p2pdma_page exists])
	])
]) # LN_IS_PCI_P2PDMA_PAGE
