...
# Based on linux/scripts/checkpatch.pl v7.0-rc5-51-gbbeb83d318
# with some additional Lustre-specific checks.
...
my $spelling_file_userspace = "$D/spelling-userspace.txt";
my $codespell = 0;
...
sub is_userspace {
	...
	$realfile =~ m@^lustre/utils/@ || $realfile =~ m@^lustre/tests@ ||
	$realfile =~ m@^lnet/utils/@ || $realfile =~ m@^contrib/@);
}
...
# Check signature styles
		if (!$in_header_lines &&
			...
			# Assisted-by uses an AGENT_NAME:MODEL_VERSION value
			if ($sign_off =~ /^Assisted-by:/i) {
				if ($email !~ /\S:\S/) {
					WARN("BAD_SIGN_OFF",
					     "Assisted-by expects 'AGENT_NAME:MODEL_VERSION [TOOL...]' format\n" . $herecurr);
				}
				next;
			}
			my ($email_name, $name_comment, $email_address, $comment) = parse_email($email);
...
# Check for Gerrit Change-Ids not in any patch context
		if (!$gerrit_ok && $realfile eq '' && !$has_patch_separator && $line =~ /^\s*change-id:/i) {
...
# otherwise an addition to them trips its own entry.
		...
		if (defined($myspell) &&
			...
			while ($rawline_utf8 =~ /(?:^|[^\w\-'`])($myspell)(?:[^\w\-'`]|$)/g) {
				...
				my $hereptr = "$hereline$ptr\n";
				my $typo_fix = $myfix{$typo};
				if (!defined($typo_fix)) {
					$typo_fix = "CHECKPATCH ERROR";
					foreach my $suspect (keys %myfix) {
						if ($typo =~ /$suspect/) {
							$typo_fix = $myfix{$suspect};
							last;
						}
					}
				}
