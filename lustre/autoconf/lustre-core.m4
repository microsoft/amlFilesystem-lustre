...
# Note: This was reimplemented by hand, since upstream's 5.19 support was against master.
#       This file was altered significantly.
#       When moving forward, check to see if this can be dropped in favor of the original patch.
#
...
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-Werror"
LB_CHECK_COMPILE([if grab_cache_page_write_begin() has flags argument],
...
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-Werror"
LB_CHECK_COMPILE([if struct address_space_operations has read_folio()],
address_space_operations_read_folio, [
		...
		AC_DEFINE(HAVE_AOPS_READ_FOLIO, 1,
			[address_spaace_operaions->read_folio() member exists])
	])
EXTRA_KCFLAGS="$tmp_flags"
]) # LC_HAVE_ADDRESS_SPACE_OPERATIONS_READ_FOLIO
...
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-Werror"
LB_CHECK_COMPILE([if read_cache_page() filler_t needs struct file],
...
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-Werror"
LB_CHECK_COMPILE([if struct address_space_operations() has release_folio()],
address_space_operations_release_folio, [
		...
		AC_DEFINE(HAVE_AOPS_RELEASE_FOLIO, 1,
			[address_spaace_operaions->release_folio() member exists])
	])
EXTRA_KCFLAGS="$tmp_flags"
]) # LC_HAVE_ADDRESS_SPACE_OPERATIONS_RELEASE_FOLIO
