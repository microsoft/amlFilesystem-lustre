...
static void __mdc_pack_body(struct mdt_body *b, __u32 suppgid)
{
	...
	b->mbo_capability = compat_capability_u32(current_cap());
}
		     ...
		     struct sptlrpc_sepol *sepol)
{
	...
	rec->cr_fsuid    = uid;
	rec->cr_fsgid    = gid;
	rec->cr_cap      = compat_capability_u32(cap_effective);
		   ...
		   size_t lmmlen, struct sptlrpc_sepol *sepol)
{
	...
	rec->cr_cap = compat_capability_u32(current_cap());
	rec->cr_mode   = mode;
				 ...
				 struct md_op_data *op_data)
{
	...
	rec->sa_cap = compat_capability_u32(current_cap());
	rec->sa_suppgid = -1;
		     ...
		     struct sptlrpc_sepol *sepol)
{
	...
	rec->ul_cap = compat_capability_u32(op_data->op_cap);
		   ...
		   struct sptlrpc_sepol *sepol)
{
	...
	rec->lk_cap      = compat_capability_u32(op_data->op_cap);
		     ...
		     struct sptlrpc_sepol *sepol)
{
	...
	rec->rn_cap      = compat_capability_u32(op_data->op_cap);
